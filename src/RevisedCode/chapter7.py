...
def translate_text(input_text, target_language="zh"):
    ...
    # 构建翻译Prompt
    messages = f"Translate the following text from {source_language} to {target_language}: \n{input_text}"
    # 调用OpenAI API进行翻译
...
class LanguageAssistance:
    ...
    def auto_complete(self, text):
        """基于部分输入内容自动补全句子。"""
        messages = f"Complete the following sentence:\n{text}"
        response = openai.ChatCompletion.create(
            engine="gpt-4-0125-preview",
            prompt=prompt,
            max_tokens=50,
    ...
    def optimize_sentence(self, text):
        """通过OpenAI模型优化句子结构。"""
        messages = f"Optimize the following sentence for clarity and conciseness:\n{text}"
        response = openai.ChatCompletion.create(
            engine="gpt-4-0125-preview",
            prompt=prompt,
            max_tokens=100,
